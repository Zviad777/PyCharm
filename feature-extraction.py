...
#უკვე დასორტირებულიდან იღებს პირველ 2 ვექტორს, სადაც ყველაზე დიდი eigenvalue ა
print("0-1", eigen_pairs[0][1], "1-1", eigen_pairs[1][1])
...
for label in range(1, 4):
    #y_train ==label ამოაგდებს ყველა სტრიქონს სადაც ეგ label არის, ხოლო მარტო label რომ გაეშვა 1 2 3 ს ამოაგდებდა და ვსიო
    mean_vecs.append(np.mean(X_train_std[y_train == label], axis=0))
    #just prints the mean vector of 13 feature values for each class(label) it is 3x13 matrice
    print('MV %s: %s\n' % (label, mean_vecs[label - 1]))


d = 13  # number of features
S_W = np.zeros((d, d))
#mean_vecs is 3x13 matrice
